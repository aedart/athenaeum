...
* Changed Http `Client` and Request `Builder`, added debugging methods (`debug()`, `dd()`, `mustDebug()`, and `mustDumpAndDie()`). This change only affects you if you have a custom Http `Client` or Request `Builder` implementation.
