...
To make use of Laravel's [Cache](https://laravel.com/docs/13.x/cache) component, you _should_ spend a bit of time configuring it.
...
See Laravel's [Redis package](https://packagist.org/packages/illuminate/redis) and it's [documentation](https://laravel.com/docs/13.x/redis) for details.

In the example illustrated below, a "user-tags" store has been added.
It relies of the Redis driver, which uses a custom "users" [connection](https://laravel.com/docs/13.x/redis#configuration).
...
You can also use Laravel's `Cache` [Facade](https://laravel.com/docs/13.x/facades), to achieve the same result.
