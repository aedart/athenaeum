...
Your legacy application most likely already has some kind of [error](https://www.php.net/manual/en/function.set-error-handler.php), [exception](https://www.php.net/manual/en/function.set-exception-handler.php) and [shutdown handling](https://www.php.net/manual/en/function.register-shutdown-function.php)[1].
Therefore, **exception handling in Athenaeum Core Application is disabled, by default**.
If it wasn't, it could conflict with your existing mechanisms.
Nevertheless, should you not be happy with your existing exception handling, then perhaps the possibilities offered by this, could prove beneficial.
