...
Inspired heavily by Laravel's [Collections](https://laravel.com/docs/8.x/collections#method-when), the `when()` and `unless()` methods give you the possibility to invoke callbacks, when a condition evaluates to `true`. 
